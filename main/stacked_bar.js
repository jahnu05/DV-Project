...
const drawStackedBars = () => {
    ...
    svg.selectAll(".ageGroup")
        ...
        .on("mouseover", function (event, d) {
            const total = d.data["Current smoker - now smokes every day"] +
            ...
            // Fade other bars
            svg.selectAll(".bar")
                .style("opacity", bar => bar.data.AgeCategory === d.data.AgeCategory ? 1 : 0.3);
        })
...
drawStackedBars();
    ...
    const drawGroupedBars = () => {
        // Draw bars
        const ageGroups = svg.selectAll(".ageGroup")
            .data(data)
            .enter().append("g")
            .attr("class", "ageGroup")
        ...
        ageGroups.selectAll("rect")
            ...
            .on("mouseover", function (event, d) {
                ...
                svg.selectAll(".bar[fill='" + currentColor + "']")
                    .style("opacity", 1); // Highlight bars of the same color
            ...
            .on("mouseout", function (d) {
                svg.selectAll(".bar")
                    .style("opacity", 1); // Restore original opacity for all bars
